...
namespace ProyectoFinalC_
{
    public partial class FormGrafos : Form
    {
        ...
        private void btnRecorridoBFS_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtOrigen.Text, out origen))
            {
                var resultadoBFS = grafo.BFS(origen);

                if (resultadoBFS.Count > 0)
                {
                    listRecorridoBFS.Items.Clear();

                    for (int i = 0; i < resultadoBFS.Count; i++)
                    {
                        List<int> paso = resultadoBFS[i];
                        string pasoStr = string.Join(" -> ", paso);
                        listRecorridoBFS.Items.Add($"Paso {i + 1}: {pasoStr}");
                    }
                }
                else
                {
                    listRecorridoBFS.Items.Clear();
                    listRecorridoBFS.Items.Add($"No se encontró un recorrido BFS desde el vértice {origen}.");
                }
            }
            else
            {
                listRecorridoBFS.Items.Clear();
                listRecorridoBFS.Items.Add("Ingrese un número válido para el vértice de inicio del recorrido BFS.");
            }
